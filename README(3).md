...
1. **Idempotency** — an advance payout must never be paid twice for the same sale, even if the payout job is triggered repeatedly.
2. **Auditability** — every rupee that moves must be traceable to a specific transaction, not just reflected in a mutable balance number.
